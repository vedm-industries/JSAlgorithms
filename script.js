...
window.onload = function () {
  let attribute = prompt(
    "What would you like your data sorted by? (rank, id, title)"
  );
...
function sortMoviesByRank(movies) {
  for (let j = 0; j < movies.length - 1; j++) {
    let max_rating = movies[j];
    let max_location = j;

    for (let i = j; i < movies.length; i++) {
      if (movies[i].rank > max_rating.rank) {
...
function getMaxMovieObject(movies, start, sortAttr) {
  ...
  return { max_rating: maximum, max_index: max_location };
}
